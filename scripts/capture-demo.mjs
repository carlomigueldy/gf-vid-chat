...
// New segmented-hero flow: reveal the Join panel, then pick the Paste method.
await joiner.getByRole('radio', { name: /join a room/i }).click()
await joiner.waitForTimeout(600) // panel swap (AnimatePresence mode="wait")
await joiner.getByRole('radio', { name: /paste/i }).click()
